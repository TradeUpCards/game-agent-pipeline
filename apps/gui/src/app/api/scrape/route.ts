...
async function startScrapingProcess(url: string, filterDepth: number, maxDepth: number | undefined, env: any, scrapyExe: string) {
  try {
    ...
    const filename = `../../data/raw-scrapes/output_${domain}_${pathSegment}_${today}.json`
    
    console.log('Generated filename:', filename)
    
    // Check if Scrapy is available
    const scrapyProjectPath = path.join(process.cwd(), '..', '..', 'tools', 'scrapy')
    const checkScrapyProcess = spawn('scrapy', ['--version'], {
      cwd: scrapyProjectPath,
    ...
    checkScrapyProcess.on('close', (code) => {
      ...
      try {
        const fs = require('fs')
        const outputPath = path.join(scrapyProjectPath, filename)
      ...
      const spiderProcess = spawn('scrapy', spiderArgs, {
        cwd: scrapyProjectPath,
        stdio: ['pipe', 'pipe', 'pipe'],
        env,
        shell: true
